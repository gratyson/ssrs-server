package com.gt.ssrs.conf.awslocal;

import com.gt.ssrs.filter.awslocal.CognitoFilter;
...
public class WebSecurityConfig {
    ...
    @Bean
    public SecurityFilterChain securityFilterChain(HttpSecurity http, CognitoFilter cognitoFilter) throws Exception {
    ...
    public ServletWebServerFactory servletContainer(@Value("${server.port.http}") int httpPort,
        ...
        tomcat.addAdditionalConnectors(redirectConnector(httpPort, httpsPort));

        if(ajpEnabled) {
            if (ajpPort > 0 && !ajpSecret.isBlank()) {
                tomcat.addAdditionalConnectors(ajpConnector(ajpPort, ajpSecret));
            } else {
